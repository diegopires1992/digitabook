...
def get_products():
    ...
    return ProductServices(session).get_all_products(), HTTPStatus.OK


@bp_products.route('/<int:product_id>', methods=['GET'])
def get_product_by_id(product_id):
    session = current_app.db.session
    return ProductServices(session).get_product_by_id(product_id)


@bp_products.route('/<int:product_id>', methods=['DELETE'])
def delete_product_by_id(product_id):
    session = current_app.db.session
    return ProductServices(session).delete_product(product_id)
...
def patch_product(product_id):
    ...
    return ProductServices(session).patch_product(product_id, data)
